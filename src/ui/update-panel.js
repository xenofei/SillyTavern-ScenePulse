...
export function updatePanel(d,_force=false){
    ...
    {const _sec=mkSection('scene','Scene Details',sceneBadge,()=>{
        ...
        {const pr=document.createElement('div');pr.className='sp-row';pr.dataset.ft='charactersPresent';pr.innerHTML=`<div class="sp-row-label">${esc('Present')}</div><div class="sp-row-value">${esc((d.charactersPresent||[]).join(', ')||'\u2014')}</div>`;f.appendChild(pr)}
        return f;
    ...
    {const _sec=mkSection('characters','Characters',d.characters?.length||0,()=>{
        ...
        {const gr=document.createElement('div');gr.className='sp-char-grid';gr.dataset.ft='char_appearance';const appearMap={hair:'char_hair',face:'char_hair',outfit:'char_outfit',stateOfDress:'char_outfit',posture:'char_posture',proximity:'char_posture',physicalState:'char_physical'};const appearFields=[['Hair','hair'],['Face','face'],['Outfit','outfit'],['Dress','stateOfDress'],['Posture','posture'],['Proximity','proximity'],['Physical','physicalState']];for(const[l,key]of appearFields){const v=ch[key]||'';const fd=document.createElement('div');fd.className='sp-char-field';fd.textContent=l;fd.dataset.ft=appearMap[key];const vd=document.createElement('div');vd.className='sp-char-val';vd.textContent=v||'\u2014';vd.dataset.ft=appearMap[key];if(!v){fd.classList.add('sp-empty-field');vd.classList.add('sp-empty-field');vd.dataset.placeholder=l}mkEditable(vd,()=>ch[key]||'',nv=>{ch[key]=nv;const snap=getLatestSnapshot();if(snap?.characters?.[ci])snap.characters[ci][key]=nv});gr.appendChild(fd);gr.appendChild(vd)}if(Array.isArray(ch.inventory)&&ch.inventory.length){const fd=document.createElement('div');fd.className='sp-char-field';fd.textContent='Inventory';fd.dataset.ft='char_inventory';const vd=document.createElement('div');vd.className='sp-char-val';vd.textContent=ch.inventory.join(', ');vd.dataset.ft='char_inventory';gr.appendChild(fd);gr.appendChild(vd)}if(gr.children.length)_cbody.appendChild(gr)}
        {const gs=document.createElement('div');gs.className='sp-char-goals';gs.dataset.ft='char_goals';const gg=document.createElement('div');gg.className='sp-char-grid';for(const[l,key]of[['Need','immediateNeed'],['Short-Term','shortTermGoal'],['Long-Term','longTermGoal']]){const v=ch[key]||'';const fd=document.createElement('div');fd.className='sp-char-field';fd.textContent=l;const vd=document.createElement('div');vd.className='sp-char-val';vd.textContent=v||'\u2014';if(!v){fd.classList.add('sp-empty-field');vd.classList.add('sp-empty-field');vd.dataset.placeholder=l}mkEditable(vd,()=>ch[key]||'',nv=>{ch[key]=nv;const snap=getLatestSnapshot();if(snap?.characters?.[ci])snap.characters[ci][key]=nv});gg.appendChild(fd);gg.appendChild(vd)}gs.appendChild(gg);_cbody.appendChild(gs)}
        // Fertility section (simplified)
        {const _isEdit=document.getElementById('sp-panel')?.classList.contains('sp-edit-mode');const _showFert=ch.fertStatus&&(ch.fertStatus!=='N/A'||_isEdit);if(_showFert||_isEdit){const fertDiv=document.createElement('div');fertDiv.className='sp-fert-section';fertDiv.dataset.ft='char_fertility';if(ch.fertStatus==='N/A'&&!_isEdit)fertDiv.innerHTML=`<div class="sp-fert-na">Fertility: N/A \u2014 ${esc(ch.fertReason||'n/a')}</div>`;else{const fg=document.createElement('div');fg.className='sp-char-grid';for(const[l,key]of[['Status','fertStatus'],['Reason','fertReason'],['Cycle','fertCyclePhase']]){const v=String(ch[key]||'');const fd=document.createElement('div');fd.className='sp-char-field';fd.textContent=l;const vd=document.createElement('div');vd.className='sp-char-val';vd.textContent=v||'\u2014';if(!v){vd.classList.add('sp-empty-field');vd.dataset.placeholder=l}mkEditable(vd,()=>String(ch[key]||''),nv=>{ch[key]=nv;const snap=getLatestSnapshot();if(snap?.characters?.[ci])snap.characters[ci][key]=nv});fg.appendChild(fd);fg.appendChild(vd)}const inlineLabel=document.createElement('div');inlineLabel.className='sp-char-field';inlineLabel.style.cssText='grid-column:1/-1;display:flex;gap:8px;flex-wrap:wrap;padding:2px 0';for(const[l,key]of[['Day','fertCycleDay'],['Window','fertWindow'],['Pregnancy','fertPregnancy'],['Week','fertPregWeek']]){const v=String(ch[key]||'');if(!v&&!_isEdit)continue;const span=document.createElement('span');span.className='sp-fert-inline';span.innerHTML=`<span class="sp-fert-inline-label">${esc(l)}</span> <span class="sp-fert-inline-val">${esc(v||'\u2014')}</span>`;mkEditable(span.querySelector('.sp-fert-inline-val'),()=>String(ch[key]||''),nv=>{ch[key]=nv;const snap=getLatestSnapshot();if(snap?.characters?.[ci])snap.characters[ci][key]=nv});inlineLabel.appendChild(span)}if(inlineLabel.children.length)fg.appendChild(inlineLabel);if(ch.fertNotes||_isEdit){const fd=document.createElement('div');fd.className='sp-char-field';fd.textContent='Notes';const vd=document.createElement('div');vd.className='sp-char-val';vd.textContent=ch.fertNotes||'\u2014';if(!ch.fertNotes){vd.classList.add('sp-empty-field');vd.dataset.placeholder='Notes'}mkEditable(vd,()=>String(ch.fertNotes||''),nv=>{ch.fertNotes=nv;const snap=getLatestSnapshot();if(snap?.characters?.[ci])snap.characters[ci].fertNotes=nv});fg.appendChild(fd);fg.appendChild(vd)}fertDiv.appendChild(fg)}_cbody.appendChild(fertDiv)}}
