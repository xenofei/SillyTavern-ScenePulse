...
function _meterHtml(key, label, value, meterLabel, prevValue) {
    ...
    // v6.15.2: meter labels capped at LLM source (MAX 3 words). Removed
    // truncateWords() — CSS ellipsis + title handles safety; chopping after
    // the fact hid prompt failures from the user's logs.
    const tagHtml = hasTag ? `<div class="sp-wiki-meter-tag" title="${esc(meterLabel)}">${esc(meterLabel)}</div>` : '';
    ...
    if (isUnknown) {
        const uTag = meterLabel ? `<div class="sp-wiki-meter-tag" title="${esc(meterLabel)}">${esc(meterLabel)}</div>` : '';
