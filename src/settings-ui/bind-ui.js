...
import { hidePanel } from '../ui/panel.js';
...
export function bindUI(){const s=getSettings();
    ...
    $('#sp-font-scale').on('input',function(){const v=+this.value;s.fontScale=v;$('#sp-font-scale-val').text(v.toFixed(1)+'x');const p=document.getElementById('sp-panel');if(p)p.style.zoom=v;const tp=document.getElementById('sp-thought-panel');if(tp)tp.style.zoom=v;saveSettings()});
    $('#sp-font-scale-reset').on('click',function(){s.fontScale=1;$('#sp-font-scale').val(1);$('#sp-font-scale-val').text('1.0x');const p=document.getElementById('sp-panel');if(p)p.style.zoom='';const tp=document.getElementById('sp-thought-panel');if(tp)tp.style.zoom='';saveSettings()});
