...
`;
    ...
    if(panels.relationships){
        ...
        if(ft.rel_milestone!==false)relFields.push('- milestone: Most recent significant moment between this NPC and {{user}}. MAX 10 words. One concrete event, not a paragraph. No commas chaining clauses, no em-dash continuations, no parenthetical asides. RIGHT: "Covered my shift when no one else would."  RIGHT: "Filed a formal complaint about a wrong order."  WRONG: "She told him coffee was never the point, she was testing his honesty, and now asks if he can keep his hands to himself for the three-block walk—a first for her on this street."  WRONG: "Walked in during the confrontation, observing quietly, and seemed to recognize the tension as familiar from her own past."');
        const meters=[];
