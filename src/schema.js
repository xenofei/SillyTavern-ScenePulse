...
`;
    ...
    if(panels.characters){
        ...
        if(ft.char_archetype!==false)charFields.push('- archetype: Single narrative role relative to {{user}}. One of: protagonist (secondary co-lead), ally (on {{user}}\'s side), rival (competitive not hostile), mentor (teaches/guides), antagonist (actively opposes), family (parent/sibling/child/spouse), love (romantic/sexual interest), incidental (minor NPC). Empty string if unclassified. Archetype can change turn-to-turn as the story develops.');
