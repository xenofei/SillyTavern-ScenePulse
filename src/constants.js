...
export const DEFAULTS=Object.freeze({enabled:true,autoGenerate:true,maxRetries:2,contextMessages:8,promptMode:'json',embedSnapshots:1,embedRole:'system',injectionMethod:'inline',deltaMode:false,functionToolEnabled:false,maxSnapshots:0,connectionProfile:'',chatPreset:'',fallbackProfile:'',fallbackPreset:'',fallbackEnabled:true,setupDismissed:false,lorebookMode:'character_attached',lorebookAllowlist:[],openSections:{scene:true,quests:true,relationships:true,characters:true,branches:false},schema:null,systemPrompt:null,showThoughts:true,thoughtPanelTruncate:false,showEmptyFields:false,thoughtPos:{x:10,y:80},devButtons:false,fontScale:1,language:'',theme:'default',sceneTransitions:true,panels:{dashboard:true,scene:true,quests:true,relationships:true,characters:true,storyIdeas:true},dashCards:{date:true,time:true,weather:true,temperature:true,location:true},fieldToggles:{},customPanels:[],charPortraits:{}});
...
- ARCHETYPE (single dominant narrative role): Pick ONE of the following for each character \u2014 the dominant narrative function they serve relative to {{user}} RIGHT NOW in the story. protagonist=secondary co-lead beside {{user}}. ally=on {{user}}'s side, supports them. rival=competitive tension, not hostile (friendly/professional rival). mentor=teaches or guides {{user}}. antagonist=actively opposes {{user}}. family=family relation (parent, sibling, child, spouse). love=romantic/sexual interest, current or prospective. incidental=minor NPC with no story weight (waiter, bartender, passing stranger). Empty string if unclassified or none fit. Archetype can change turn-to-turn as the story develops \u2014 a stranger becomes an ally, an ally becomes a rival. Pick the role that MATTERS MOST this turn, even if the character has multiple functions (e.g. a love interest who is also the antagonist \u2192 pick "love" if the current scene is about the romance, "antagonist" if it's about the conflict).
...
- innerThought: The exact sentence running through this character's head right now, written as a first-person thought. 1-3 sentences. Write it as dialogue they're having with themselves \u2014 use their voice, their word choices, their cadence. BE them for one sentence. RIGHT: "Oh god. Please don't stop. If she hears us... I don't care anymore." WRONG (this is a description of feelings, not a thought): "Overwhelmed, desperate, euphoric." If the character would say "I'm scared" in their head, that's a valid thought. But "scared, anxious, panicked" as a bare list of labels is NOT a thought \u2014 it's you describing them from outside.
