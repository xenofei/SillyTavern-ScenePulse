...
'Russian': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'Переключить мысли','Toggle weather overlay':'Переключить погодный оверлей','Toggle time-of-day ambience':'Переключить освещение времени суток','Toggle location change popups':'Переключить всплывающие окна смены локации',
    'Enable Schema Editing':'Разрешить редактирование схемы','Schema editing is locked to prevent accidental changes.':'Редактирование схемы заблокировано для предотвращения случайных изменений.',
    'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.':'Редактирование JSON-схемы может сломать генерацию трекера, если схема станет невалидной. Изменяйте только если знаете, что делаете.',
    'No lorebooks detected':'Книги знаний не обнаружены','Using recommended:':'Используется рекомендуемый:','Recommended:':'Рекомендуется:','Apply':'Применить',
    'The instruction sent to the model. Defines how the tracker generates data.':'Инструкция, отправляемая модели. Определяет, как трекер генерирует данные.',
    'The JSON schema defining the output structure. Must be valid JSON.':'JSON-схема, определяющая структуру вывода. Должна быть валидным JSON.',
    'Removes all tracker snapshots from this chat. Settings preserved.':'Удаляет все снимки трекера из этого чата. Настройки сохраняются.',
    'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.':'Сбрасывает метод вставки, профили, режим книг знаний и т.д. Данные трекера сохраняются.',
    'ScenePulse internal debug log (normalizer, extraction, generation).':'Внутренний журнал отладки ScenePulse (нормализация, извлечение, генерация).',
    'Full browser console output including SillyTavern and other extensions.':'Полный вывод консоли браузера, включая SillyTavern и другие расширения.',
    'Raw tracker JSON from the most recent generation.':'Необработанный JSON трекера из последней генерации.',
    'In Separate mode, this is the full API response. In Together mode, this is the extracted JSON block that was stripped from the AI\'s narrative response.':'В режиме «Раздельно» — полный ответ API. В режиме «Совместно» — извлечённый блок JSON, удалённый из повествовательного ответа ИИ.',
    'The AI writes its normal response, then appends tracker JSON at the end. ScenePulse automatically extracts and hides the JSON.':'ИИ пишет обычный ответ, затем добавляет JSON трекера в конце. ScenePulse автоматически извлекает и скрывает JSON.',
    'If the AI omits the tracker payload, ScenePulse can automatically run a separate API call to recover. Requires a connection profile to be configured.':'Если ИИ пропустит данные трекера, ScenePulse может автоматически выполнить отдельный API-вызов для восстановления. Требуется настроенный профиль подключения.',
    'Dramatic':'Драматический','Intense':'Интенсивный','Comedic':'Комедийный','Twist':'Поворот','Exploratory':'Исследовательский',
...
'Japanese': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'思考を切替','Toggle weather overlay':'天気オーバーレイを切替','Toggle time-of-day ambience':'時間帯の雰囲気を切替','Toggle location change popups':'場所変更ポップアップを切替',
    'Enable Schema Editing':'スキーマ編集を有効化','Schema editing is locked to prevent accidental changes.':'誤った変更を防ぐためスキーマ編集はロックされています。',
    'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.':'JSONスキーマの編集はスキーマが無効になるとトラッカー生成を壊す可能性があります。理解している場合のみ変更してください。',
    'No lorebooks detected':'ロアブックが検出されません','Using recommended:':'推奨設定使用中:','Recommended:':'推奨:','Apply':'適用',
    'The instruction sent to the model. Defines how the tracker generates data.':'モデルに送信される指示。トラッカーのデータ生成方法を定義します。',
    'The JSON schema defining the output structure. Must be valid JSON.':'出力構造を定義するJSONスキーマ。有効なJSONである必要があります。',
    'Removes all tracker snapshots from this chat. Settings preserved.':'このチャットのトラッカースナップショットをすべて削除します。設定は保持されます。',
    'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.':'注入方法、プロファイル、ロアブックモードなどをデフォルトにリセットします。トラッカーデータは保持されます。',
    'ScenePulse internal debug log (normalizer, extraction, generation).':'ScenePulse内部デバッグログ（正規化、抽出、生成）。',
    'Full browser console output including SillyTavern and other extensions.':'SillyTavernや他の拡張機能を含むブラウザコンソールの全出力。',
    'Raw tracker JSON from the most recent generation.':'最新の生成からの未加工トラッカーJSON。',
    'Dramatic':'ドラマチック','Intense':'激しい','Comedic':'コメディ','Twist':'ツイスト','Exploratory':'探索的',
...
'Korean': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'생각 전환','Toggle weather overlay':'날씨 오버레이 전환','Toggle time-of-day ambience':'시간대 분위기 전환','Toggle location change popups':'위치 변경 팝업 전환',
    'Enable Schema Editing':'스키마 편집 활성화','Schema editing is locked to prevent accidental changes.':'실수로 인한 변경을 방지하기 위해 스키마 편집이 잠겨 있습니다.',
    'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.':'JSON 스키마 편집 시 스키마가 유효하지 않게 되면 트래커 생성이 중단될 수 있습니다. 무엇을 하는지 알고 있는 경우에만 수정하세요.',
    'No lorebooks detected':'로어북이 감지되지 않음','Using recommended:':'권장 사용 중:','Recommended:':'권장:','Apply':'적용',
    'The instruction sent to the model. Defines how the tracker generates data.':'모델에 전송되는 지시. 트래커가 데이터를 생성하는 방법을 정의합니다.',
    'The JSON schema defining the output structure. Must be valid JSON.':'출력 구조를 정의하는 JSON 스키마. 유효한 JSON이어야 합니다.',
    'Removes all tracker snapshots from this chat. Settings preserved.':'이 채팅의 모든 트래커 스냅샷을 삭제합니다. 설정은 보존됩니다.',
    'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.':'주입 방법, 프로필, 로어북 모드 등을 기본값으로 재설정합니다. 트래커 데이터는 보존됩니다.',
    'Dramatic':'극적','Intense':'격렬','Comedic':'코미디','Twist':'반전','Exploratory':'탐색적',
...
'Chinese (Simplified)': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'切换想法','Toggle weather overlay':'切换天气覆盖','Toggle time-of-day ambience':'切换时间段氛围','Toggle location change popups':'切换位置变更弹窗',
    'Enable Schema Editing':'启用模式编辑','Schema editing is locked to prevent accidental changes.':'模式编辑已锁定以防止意外更改。',
    'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.':'编辑JSON模式可能会在模式无效时破坏追踪器生成。仅在您知道自己在做什么时才修改。',
    'No lorebooks detected':'未检测到知识库','Using recommended:':'使用推荐:','Recommended:':'推荐:','Apply':'应用',
    'The instruction sent to the model. Defines how the tracker generates data.':'发送给模型的指令。定义追踪器如何生成数据。',
    'The JSON schema defining the output structure. Must be valid JSON.':'定义输出结构的JSON模式。必须是有效的JSON。',
    'Removes all tracker snapshots from this chat. Settings preserved.':'删除此聊天中的所有追踪器快照。设置保留。',
    'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.':'将注入方法、配置、知识库模式等重置为默认值。追踪器数据保留。',
    'Dramatic':'戏剧性','Intense':'激烈','Comedic':'喜剧','Twist':'转折','Exploratory':'探索性',
...
'Spanish': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'Alternar pensamientos','Toggle weather overlay':'Alternar superposición del clima','Toggle time-of-day ambience':'Alternar ambiente horario','Toggle location change popups':'Alternar ventanas de cambio de ubicación',
    'Enable Schema Editing':'Habilitar edición de esquema','Schema editing is locked to prevent accidental changes.':'La edición del esquema está bloqueada para prevenir cambios accidentales.',
    'No lorebooks detected':'No se detectaron lorebooks','Using recommended:':'Usando recomendado:','Recommended:':'Recomendado:','Apply':'Aplicar',
    'Dramatic':'Dramático','Intense':'Intenso','Comedic':'Cómico','Twist':'Giro','Exploratory':'Exploratorio',
...
'French': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'Basculer les pensées','Toggle weather overlay':'Basculer la superposition météo','Toggle time-of-day ambience':'Basculer l\'ambiance horaire','Toggle location change popups':'Basculer les popups de changement de lieu',
    'Enable Schema Editing':'Activer la modification du schéma','Schema editing is locked to prevent accidental changes.':'La modification du schéma est verrouillée pour éviter les changements accidentels.',
    'No lorebooks detected':'Aucun lorebook détecté','Using recommended:':'Utilisation recommandée :','Recommended:':'Recommandé :','Apply':'Appliquer',
    'Dramatic':'Dramatique','Intense':'Intense','Comedic':'Comique','Twist':'Rebondissement','Exploratory':'Exploratoire',
    'None suggested yet':'Aucune suggestion pour le moment',
},
'German': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'Gedanken umschalten','Toggle weather overlay':'Wetter-Overlay umschalten','Toggle time-of-day ambience':'Tageszeit-Ambiente umschalten','Toggle location change popups':'Standortwechsel-Popups umschalten',
    'Enable Schema Editing':'Schema-Bearbeitung aktivieren','Schema editing is locked to prevent accidental changes.':'Die Schema-Bearbeitung ist gesperrt, um versehentliche Änderungen zu verhindern.',
    'No lorebooks detected':'Keine Lorebooks erkannt','Using recommended:':'Empfohlene Einstellung:','Recommended:':'Empfohlen:','Apply':'Anwenden',
    'Dramatic':'Dramatisch','Intense':'Intensiv','Comedic':'Komödiantisch','Twist':'Wendung','Exploratory':'Erkundend',
...
'Portuguese': {
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'Toggle thoughts','Toggle weather overlay':'Toggle weather overlay','Toggle time-of-day ambience':'Toggle time-of-day ambience','Toggle location change popups':'Toggle location change popups',
    'Enable Schema Editing':'Enable Schema Editing','Schema editing is locked to prevent accidental changes.':'Schema editing is locked to prevent accidental changes.',
    'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.':'Editing the JSON schema can break tracker generation if the schema becomes invalid. Only modify if you know what you\'re doing.',
    'No lorebooks detected':'No lorebooks detected','Using recommended:':'Using recommended:','Recommended:':'Recommended:','Apply':'Apply',
    'The instruction sent to the model. Defines how the tracker generates data.':'The instruction sent to the model. Defines how the tracker generates data.',
    'The JSON schema defining the output structure. Must be valid JSON.':'The JSON schema defining the output structure. Must be valid JSON.',
    'Removes all tracker snapshots from this chat. Settings preserved.':'Removes all tracker snapshots from this chat. Settings preserved.',
    'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.':'Resets injection method, profiles, lorebook mode, etc. to defaults. Tracker data preserved.',
    'ScenePulse internal debug log (normalizer, extraction, generation).':'ScenePulse internal debug log (normalizer, extraction, generation).',
    'Full browser console output including SillyTavern and other extensions.':'Full browser console output including SillyTavern and other extensions.',
    'Raw tracker JSON from the most recent generation.':'Raw tracker JSON from the most recent generation.',
    'In Separate mode, this is the full API response. In Together mode, this is the extracted JSON block that was stripped from the AI\'s narrative response.':'In Separate mode, this is the full API response. In Together mode, this is the extracted JSON block that was stripped from the AI\'s narrative response.',
    'The AI writes its normal response, then appends tracker JSON at the end. ScenePulse automatically extracts and hides the JSON.':'The AI writes its normal response, then appends tracker JSON at the end. ScenePulse automatically extracts and hides the JSON.',
    'If the AI omits the tracker payload, ScenePulse can automatically run a separate API call to recover. Requires a connection profile to be configured.':'If the AI omits the tracker payload, ScenePulse can automatically run a separate API call to recover. Requires a connection profile to be configured.',
    'After the AI responds, a separate quiet API call generates the tracker JSON independently. Expect ~250\u2013300s total per message (narrative + wait + tracker).':'After the AI responds, a separate quiet API call generates the tracker JSON independently. Expect ~250\u2013300s total per message (narrative + wait + tracker).',
    '\u2713 Single API call \u2014 typically ~100\u2013120s total':'\u2713 Single API call \u2014 typically ~100\u2013120s total',
    '\u2713 No profile switching \u2014 eliminates message deletion risk':'\u2713 No profile switching \u2014 eliminates message deletion risk',
    '\u2713 AI has full narrative context for accurate tracking':'\u2713 AI has full narrative context for accurate tracking',
    '\u2713 2\u20133\u00d7 faster than Separate mode in practice':'\u2713 2\u20133\u00d7 faster than Separate mode in practice',
    '\u2717 Uses tokens from the main response budget (~1,700 tokens for tracker)':'\u2717 Uses tokens from the main response budget (~1,700 tokens for tracker)',
    '\u2717 May slightly reduce narrative length on token-limited models':'\u2717 May slightly reduce narrative length on token-limited models',
    '\u2713 Clean responses \u2014 narrative never token-competes with tracker':'\u2713 Clean responses \u2014 narrative never token-competes with tracker',
    '\u2713 Dedicated token budget for tracker output':'\u2713 Dedicated token budget for tracker output',
    '\u2713 Can use a different connection profile/preset':'\u2713 Can use a different connection profile/preset',
    '\u2717 Two API calls per message \u2014 2\u20133\u00d7 slower than Together mode':'\u2717 Two API calls per message \u2014 2\u20133\u00d7 slower than Together mode',
    '\u2717 ~12s dead time between calls (chat save + preset switching)':'\u2717 ~12s dead time between calls (chat save + preset switching)',
    '\u2717 Tracker prompt is ~10K+ tokens at 8 context msgs \u2014 reduce to 3\u20134 for speed':'\u2717 Tracker prompt is ~10K+ tokens at 8 context msgs \u2014 reduce to 3\u20134 for speed',
    '\u2717 Profile switching can cause message deletion (race condition with other extensions)':'\u2717 Profile switching can cause message deletion (race condition with other extensions)',
    '\u2717 Embeds previous snapshot into narrative call (~1.9K tokens) unless Embed snapshots = 0':'\u2717 Embeds previous snapshot into narrative call (~1.9K tokens) unless Embed snapshots = 0',
    'How many recent messages to include when generating tracker updates.':'How many recent messages to include when generating tracker updates.',
    'Separate mode only \u2014 Together mode uses ST\'s full context automatically.':'Separate mode only \u2014 Together mode uses ST\'s full context automatically.',
    'Fastest. Good for simple 1-on-1 scenes (~5K token prompt)':'Fastest. Good for simple 1-on-1 scenes (~5K token prompt)',
    'Balanced. Recommended for most scenes (~8\u201312K tokens)':'Balanced. Recommended for most scenes (~8\u201312K tokens)',
    'Better continuity for complex multi-character scenes (~12\u201320K tokens)':'Better continuity for complex multi-character scenes (~12\u201320K tokens)',
    'Maximum context but significantly slower and more expensive':'Maximum context but significantly slower and more expensive',
    'This is the biggest factor in Separate mode speed. At 8 msgs your tracker prompt is ~10K tokens \u2014 doubling roughly doubles generation time. Lower values (3\u20134) can cut tracker time by 40\u201360%.':'This is the biggest factor in Separate mode speed. At 8 msgs your tracker prompt is ~10K tokens \u2014 doubling roughly doubles generation time. Lower values (3\u20134) can cut tracker time by 40\u201360%.',
    'Separate mode only.':'Separate mode only.','How many times to retry if the tracker API call returns invalid JSON.':'How many times to retry if the tracker API call returns invalid JSON.',
    'Built-in preset: temp=0.6, top_p=0.95, freq_pen=0.15, max_tokens=4096. Optimized for structured JSON output on GLM-5.':'Built-in preset: temp=0.6, top_p=0.95, freq_pen=0.15, max_tokens=4096. Optimized for structured JSON output on GLM-5.',
    'Embed recent scene snapshots into the narrative conversation context so the AI can reference tracker state while writing. Only applies in Separate mode. The tracker\'s own API call always receives the previous snapshot regardless of this setting.':'Embed recent scene snapshots into the narrative conversation context so the AI can reference tracker state while writing. Only applies in Separate mode. The tracker\'s own API call always receives the previous snapshot regardless of this setting.',
    'Each embedded snapshot adds ~1.9K tokens to your narrative prompt. Set to 0 to eliminate this overhead \u2014 the tracker API call still receives previous state independently. Set to 1 for narrative continuity (AI remembers scene details). Values above 1 are rarely beneficial.':'Each embedded snapshot adds ~1.9K tokens to your narrative prompt. Set to 0 to eliminate this overhead \u2014 the tracker API call still receives previous state independently. Set to 1 for narrative continuity (AI remembers scene details). Values above 1 are rarely beneficial.',
    'Injected as invisible context \u2014 AI treats it as authoritative background info. Best for continuity without polluting the conversation.':'Injected as invisible context \u2014 AI treats it as authoritative background info. Best for continuity without polluting the conversation.',
    'Appears as if the user said it \u2014 some models respond more attentively.':'Appears as if the user said it \u2014 some models respond more attentively.',
    'Appears as previous AI output \u2014 can reinforce the AI\'s own memory but may confuse some models.':'Appears as previous AI output \u2014 can reinforce the AI\'s own memory but may confuse some models.',
    'Separate generation runs an isolated API call \u2014 it needs lorebook context injected since ST won\'t provide it automatically.':'Separate generation runs an isolated API call \u2014 it needs lorebook context injected since ST won\'t provide it automatically.',
    'Together mode uses the normal generation \u2014 ST already injects lorebooks into context, so including them here would be redundant.':'Together mode uses the normal generation \u2014 ST already injects lorebooks into context, so including them here would be redundant.',
    'Dramatic':'Dramatic','Intense':'Intense','Comedic':'Comedic','Twist':'Twist','Exploratory':'Exploratory',
    'None suggested yet':'None suggested yet',
},
'Chinese (Traditional)': {
    // Section headers
    ...
    // Schema & settings hints (v5.8.4)
    'Toggle thoughts':'切換想法','Toggle weather overlay':'切換天氣覆蓋','Toggle time-of-day ambience':'切換時間段氛圍','Toggle location change popups':'切換位置變更彈窗',
    'Enable Schema Editing':'啟用模式編輯','Schema editing is locked to prevent accidental changes.':'模式編輯已鎖定以防止意外更改。',
    'No lorebooks detected':'未偵測到知識庫','Using recommended:':'使用推薦:','Recommended:':'推薦:','Apply':'套用',
    'Dramatic':'戲劇性','Intense':'激烈','Comedic':'喜劇','Twist':'轉折','Exploratory':'探索性',
    'None suggested yet':'尚無建議',
},
'Hindi': {
    // Section headers
