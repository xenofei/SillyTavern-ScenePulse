// ScenePulse v4.9.46 — Side Panel Architecture
...
function spUpdateFab(){
    ...
    if(!fab){console.log(LOG,'FAB: element not found');return}
    ...
    console.log(LOG,'FAB:','mode='+mode,'panelVisible='+panelVis,'shouldShow='+shouldShow);
    if(shouldShow){
    ...
    console.log(LOG,'FAB post:','display='+cs.display,'zIndex='+cs.zIndex,'bottom='+cs.bottom,'right='+cs.right,'vis='+cs.visibility,'opacity='+cs.opacity,'classes='+fab.className);
}
...
function getSettings(){const{extensionSettings}=SillyTavern.getContext();if(!extensionSettings[MODULE_NAME])extensionSettings[MODULE_NAME]=structuredClone(DEFAULTS);const s=extensionSettings[MODULE_NAME];for(const k of Object.keys(DEFAULTS))if(!Object.hasOwn(s,k))s[k]=DEFAULTS[k];return s}
...
// ── External Access ──
function getConnectionProfiles(){try{const o=document.querySelectorAll('#connection_profile option');if(o.length)return Array.from(o).filter(x=>x.value).map(x=>({id:x.value,name:x.textContent.trim()}))}catch(e){warn('Profiles:',e)}return[]}
...
async function withProfileAndPreset(pid,pre,fn){
    ...
    if(pid){try{pp=document.querySelector('#connection_profile')?.value;if(typeof ctx.setConnectionProfile==='function')await ctx.setConnectionProfile(pid);else{const s=document.querySelector('#connection_profile');if(s){s.value=pid;s.dispatchEvent(new Event('change'));await new Promise(r=>setTimeout(r,300))}}}catch(e){warn('Profile:',e)}}
    ...
    try{return await fn()}finally{
        ...
        if(pp){try{if(typeof ctx.setConnectionProfile==='function')await ctx.setConnectionProfile(pp);else{const s=document.querySelector('#connection_profile');if(s){s.value=pp;s.dispatchEvent(new Event('change'))}}}catch{}}
    }
}
...
function cancelGeneration(){
    ...
    generating=false; // unlock for next generation
...
function startStreamingHider(){
    ...
    _streamHiderInterval=setInterval(()=>{
        try{
            ...
            if(timeIdx<50)return;
            let jsonStart=-1;
            for(let i=timeIdx;i>=0;i--){if(text[i]==='{'){jsonStart=i;break}}
            if(jsonStart<50)return;
            const cutPos=jsonStart;
            ...
            while(node=walker.nextNode()){
                ...
                if(charCount+len>cutPos){targetNode=node;targetOffset=cutPos-charCount;break}
                charCount+=len;
            }
            ...
            if(narrativeHeight>30&&_streamHiderStyleEl){
                ...
                // CSS rule targeting the specific message — survives innerHTML AND DOM recreation
...
function stopStreamingHider(){
    ...
    // Remove the CSS rule — must happen AFTER extraction has cleaned the DOM
    if(_streamHiderStyleEl){_streamHiderStyleEl.remove();_streamHiderStyleEl=null}
}
...
async function generateTracker(mesIdx,partKey,opts){
    ...
    generating=true;cancelRequested=false;
    ...
    generating=false;cancelRequested=false;cleanupGenUI();
    if(result){
...
function createPanel(){
    ...
    <div class="sp-toolbar">
        <div class="sp-brand-icon" id="sp-brand-icon" title="ScenePulse v4.9.46">${MASCOT_SVG}</div>
...
function clearWeatherOverlay(){
    ...
    if(ov){ov.innerHTML='';ov.className='sp-wx-ov';currentWeatherType=''}
}
...
function clearTimeTint(){
    const ov=document.getElementById('sp-time-tint');if(ov)ov.className='sp-time-tint';currentTimePeriod='';
}
...
function updatePanel(d,_force=false){
    ...
    // Timeline scrubber — shows all snapshot positions
    renderTimeline();
...
function mkSection(key,title,badge,fn,s){
    ...
    // Click anywhere on header toggles, except refresh button
...
function updateThoughts(d){
    ...
    for(const ch of sortedTpChars){
        ...
        if(thought){
            // Split into sentences and pick a stable pseudo-random limit of 1-3
            // Uses thought content hash so the same thought always shows the same count
            const sentences=thought.match(/[^.!?]+[.!?]+/g)||[thought];
            let th=0;for(let i=0;i<thought.length;i++)th=((th<<5)-th+thought.charCodeAt(i))|0;
            const sentenceLimit=(Math.abs(th)%3)+1; // 1, 2, or 3 — stable per thought
            const limited=sentences.slice(0,sentenceLimit).join(' ').trim();
            html+=`<div class="sp-tp-monologue">${esc(limited)}</div>`;
        }
        // Goals below
        // Thought bubble shows ONLY inner dialogue - no goals
        card.innerHTML=html;
...
function loadUI(){const s=getSettings();$('#sp-enabled').prop('checked',s.enabled);$('#sp-auto-gen').prop('checked',s.autoGenerate);$('#sp-show-thoughts').prop('checked',s.showThoughts!==false);$('#sp-show-weather').prop('checked',s.weatherOverlay!==false);$('#sp-show-timetint').prop('checked',s.timeTint!==false);$('#sp-show-devbtns').prop('checked',s.devButtons===true);$('#sp-ctx').val(s.contextMessages);$('#sp-retries').val(s.maxRetries);$('#sp-profile').val(s.connectionProfile||'');$('#sp-preset').val(s.chatPreset||'');$('#sp-mode').val(s.promptMode||'json');$('#sp-embed-n').val(s.embedSnapshots);$('#sp-embed-role').val(s.embedRole);$('#sp-lore-mode').val(s.lorebookMode||'include_all');
    // Fallback settings
    $('#sp-fallback-enabled').prop('checked',s.fallbackEnabled!==false);
    $('#sp-fallback-profile').val(s.fallbackProfile||'');
    $('#sp-fallback-preset').val(s.fallbackPreset||'');
